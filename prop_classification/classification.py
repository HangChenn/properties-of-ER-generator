...
def SBM_same_den_diff_p_v():
	...
	# path = "./generated_data/SBM_V=100/num_p=10/p=0.3"
	path = "./generated_data/SBM_V=100/num_p=20/p=0.5"	
	# path = "./generated_data/mix_generators"
	...
	# props_list_list = get_selected_props_list(len(X_fea[0]), 1)
	# data_result = []
	...
	for task in range(1,6):
		performance.append(use_machine_learning(X,y,task))
	# use_embedding(X,y)


	performance = np.asarray(performance).transpose()
	print(performance)
	label_list = ['Random Forest', 'Logistic Regression', 'SVM', 'NN']
	corresponding_features_name = ['density', '2 feas', 'deg seq', '15 feas',  'all']
	# corresponding_features_name = ['density', '2 feas', '15 feas',  'all']
	for ml,label in zip(performance,label_list):
		plt.plot(range(len(corresponding_features_name)), ml, label=label)
	plt.ylim(0,1)

	plt.xticks(range(len(corresponding_features_name)), corresponding_features_name)
	plt.xlabel('features type')
	plt.ylabel('accuracy')
	plt.title("SBM performance with same expect density and many p matrix")
...
def modify_features(X, task):
	...
	if task == 2:
		...
		X_fea = np.column_stack((X[:,0], X[:,11]))
	if task == 3:
